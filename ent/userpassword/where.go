...
func StatusEQ(v types.SimpleStatus) predicate.UserPassword {
	vc := v
	return predicate.UserPassword(sql.FieldEQ(FieldStatus, vc))
}

// StatusNEQ applies the NEQ predicate on the "status" field.
func StatusNEQ(v types.SimpleStatus) predicate.UserPassword {
	vc := v
	return predicate.UserPassword(sql.FieldNEQ(FieldStatus, vc))
}

// StatusIn applies the In predicate on the "status" field.
func StatusIn(vs ...types.SimpleStatus) predicate.UserPassword {
	v := make([]any, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
...
func StatusNotIn(vs ...types.SimpleStatus) predicate.UserPassword {
	v := make([]any, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
